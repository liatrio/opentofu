...
import (
	...
	"github.com/opentofu/opentofu/internal/filesys"
)
...
func extractHost(n string) (string, string, error) {
	...
	if host == "" {
		return "", "", fmt.Errorf("failed to parse host segment")
	}
...
func FromResourceURL(url string) (*git.Repository, *RefMetadata, error) {
	...
	hash, err := r.ResolveRevision(plumbing.Revision(spec.Ref))
	if err != nil {
		return nil, nil, err
	}

	_, err = r.Tag(spec.Ref)
	isTag := (err == nil)

	noGit := strings.Replace(spec.RepoPath, ".git", "", -1)

	argo := RefMetadata{
		Host:      spec.Host,
		Namespace: filepath.Dir(spec.RepoPath),
		Repo:      filepath.Base(noGit),
		Sha:       hash.String(),
		Ref:       spec.Ref,
		Tag:       isTag,
		Directory: spec.KustRootPath,
	}
